...
pair<DataFrame,vector<size_t>> kmeans(const DataFrame& data, size_t k) {//const DataFrame& cluster
  ...
  for (size_t it = 0; it < iterations; it++) {
    // Find assignments
    #pragma omp parallel for num_threads(p) schedule(static) //reduction(+:enlinea)
    for (size_t point = 0; point < data.size(); point++) {
      double best_distance = numeric_limits<double>::max();
      size_t best_cluster = 0;
      for(size_t clu = 0; clu < k; clu++) {
        double distance = sdistance(data[point], means[clu]);
...
double ssd(const DataFrame& data,const DataFrame& means,const vector<size_t> assignments){
  double ssd=0;
  /*
  la suma de todas las distancias de los puntos a sus clusters
  */
  int p= omp_get_num_procs() * 2;//2;
  #pragma omp parallel for num_threads(p) schedule(static) reduction(+:ssd)
  for(int i = 0 ; i<data.size();i++){
    ssd += sdistance(data[i],means[int(assignments[i])]);
  }
  //assert(ssd>0);
  return ssd;
}
...
int main(int argc, const char* argv[]) {
  ...
  //}
  cerr << "Took: " << total_elapsed << " s" << endl; // / number_of_runs << "s (" << number_of_runs << " runs)" << endl;
  cout<< "\nk: "<< k <<" ssd :" << ssd(data,c,a)<<endl;
  //printStats(k, a, c);
